...
- Два источника заявок и разная семантика цены: Greenwich получает динамический коэффициент по уровню рейтинга, внешняя заявка — полную цену. Сервер всегда перепроверяет уровень при создании заявки.
