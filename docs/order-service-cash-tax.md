...
## Редактирование на этапах заявки

Склад может менять дополнительные услуги на любом этапе заявки, кроме отменённой (`CANCELLED`), включая выданную, находящуюся на приёмке и закрытую заявку. В специальном режиме редактируются:

- включение и отключение доставки, монтажа и демонтажа;
- комментарий и цена услуги для клиента;
- внутренняя себестоимость и способ её оплаты;
- скрытые внутренние траты.

Такое изменение не затрагивает статус, даты, реквизит и складской резерв. Клиентский итог и аналитика пересчитываются из актуальных полей `Order`. Если у заявки уже есть файл сметы, клиентские изменения доп. услуг пересобирают текущий XLSX, не переписывая исторические snapshots согласования.
