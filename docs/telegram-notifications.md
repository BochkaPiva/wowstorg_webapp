...
Ежедневный runner проверяет основные активные заявки Greenwich и за **30, 7 и 3 дня** до начала аренды отправляет назначенному сотруднику персональное сообщение с кнопками:

- **Всё актуально** — подтверждение сохраняется без уведомления склада;
...
Каждая контрольная точка записывается в `GreenwichOrderReminder`. Неуспешная отправка остаётся в очереди и повторяется следующим ежедневным запуском. Если сотруднику Greenwich не привязан личный Telegram, склад получает одно служебное предупреждение вместо потери напоминания.
